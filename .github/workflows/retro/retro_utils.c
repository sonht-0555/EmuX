...
emux_is_dirty(const uint32_t *restrict buf1, uint32_t *restrict buf2,
              size_t size) {
  size_t len = size >> 2;
  size_t step = len >> 8;
  if (step) {
...
EMSCRIPTEN_KEEPALIVE __attribute__((used)) int
emux_render16(const uint16_t *restrict src, uint16_t *restrict cache,
              uint32_t *restrict dst, int width, int height, int stride,
              const uint32_t *restrict lut) {
  ...
  if (memcmp(src, cache, total_px << 1) == 0)
    return 0;
dirty16:
...
EMSCRIPTEN_KEEPALIVE __attribute__((used)) int
emux_render32(const uint32_t *restrict src, uint32_t *restrict cache,
              uint32_t *restrict dst, int length) {
  ...
  if (memcmp(src, cache, length << 2) == 0)
    return 0;
dirty32:
...
EMSCRIPTEN_KEEPALIVE __attribute__((used)) float *emux_audio_get_buffer_l() {
  return audio_out_l;
}
EMSCRIPTEN_KEEPALIVE __attribute__((used)) float *emux_audio_get_buffer_r() {
  return audio_out_r;
}
...
EMSCRIPTEN_KEEPALIVE __attribute__((used)) int
emux_audio_process(const int16_t *src, int frames) {
  if (frames <= 0)
    return 0;
