...
/* Static storage for legacy memstream (PokeMini style - 2 args memstream_set_buffer) */
...
/* --- Memstream API for PokeMini (Legacy 2-arg style) --- */
...
WEAK int64_t memstream_write(void *stream, const void *data, uint64_t len) {
  struct memstream *m = (struct memstream *)stream;
...
WEAK int64_t memstream_read(void *stream, void *data, uint64_t len) {
  struct memstream *m = (struct memstream *)stream;
