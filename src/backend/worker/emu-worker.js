...
self.onmessage = async (e) => {
    ...
    if (type === 'INIT') {
        const { config, romData, romName, wasmUrl, jsUrl, isArcade, isNDS,
                canvas, canvasBottom, renderer, sabInput, audioSABs, biosFiles } = data;
        ...
        self.Module = {
            ...
            async onRuntimeInitialized() {
                ...
                Module._retro_init();
                // Write BIOS files to Emscripten FS
                if (biosFiles && biosFiles.length > 0) {
                    biosFiles.forEach(f => Module.FS.writeFile('/' + f.name, f.data));
                }
    ...
    if (type === 'CHANGE_RENDERER') {
        self.rendererName = data;
        renderFunction = null; rendererReady = false;
        importScripts('./worker-video.js');
        scriptName = data;
    }
    if (type === 'SAVE_STATE') {
        try {
            ...
            if (Module._retro_serialize(ptr, size)) {
                self.postMessage({ type: 'STATE_DATA', data: { state: new Uint8Array(Module.HEAPU8.buffer, ptr, size).slice(), slot: data.slot } });
            }
            Module._free(ptr);
