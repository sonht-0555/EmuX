...
async function extractCBZ(arrayBuffer, romName) {
    ...
    const manga = document.createElement('manga'), one = document.createElement('one'), two = document.createElement('two'), num = document.createElement('num'), transWrap = document.createElement('div'), trans = document.createElement('trans'), tHeader = document.createElement('div'), tFooter = document.createElement('div');
    transWrap.className = 'trans-wrap'; tHeader.className = 't-header'; tFooter.className = 't-footer'; transWrap.hidden = true;
    transWrap.append(tHeader, trans, tFooter); transData ? screen.append(num, one, two, manga, transWrap) : screen.append(num, one, two, manga);
    const updateTrans = p => {
    ...
    const loadNext = amount => {let html = ''; for (let end = Math.min(loadedCount + amount, urls.length); loadedCount < end; loadedCount++) html += `<img style="pointer-events:none" src="${urls[loadedCount]}" loading="lazy" data-index="${loadedCount}" image-name="${displayNames[loadedCount].split('/').pop().replace(/\.[^/.]+$/, "")}">`; manga.insertAdjacentHTML('beforeend', html);};
    ...
    num.onclick = e => {e.stopPropagation(); let p = prompt("Page:", page + 1) - 1; if (p >= 0 && p < urls.length) {if (p >= loadedCount) loadNext(p - loadedCount + 2); let t = manga.children[p]; if (t) manga.scrollTop += t.getBoundingClientRect().top - manga.getBoundingClientRect().top; local(`page_${romName}`, page = p); num.innerHTML = `${page + 1}|${urls.length}`; updateTrans(page);} };
    await showNotification("", "###", "", "", true);
    ...
    manga.onpointerdown = event => {
        ...
        click(() => {if (t) {let i = t.getAttribute('data-index'); if (i != null) {local(`page_${romName}`, page = Number(i)); num.innerHTML = `${page + 1}|${urls.length}`; updateTrans(page);} } }, null, () => {let newLevel = prompt("Level:", level); if (newLevel != null) setMangaLevel(newLevel, true);});
    };
}
