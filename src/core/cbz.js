...
async function extractCBZ(arrayBuffer, romName) {
    ...
    const allNames = Object.keys(files).filter(n => (imgExts.test(n) || n.endsWith('.scan') || n.includes('.scan.')) && files[n].length > 0).sort((a, b) => naturalNameCollator.compare(a, b)), displayNames = [], nameToUrl = {};
