...
const CORE_CONFIG = [
    ...
    {ext: '.zip', script: CORE_BASE + 'arcade.zip', btns: {'btn-1': ['A', 0], 'btn-3': ['B', 8], 'btn-2': ['C', 1], 'btn-4': ['D', 9], 'btn-l': [' bl.', ''], 'btn-r': [' br.', ''], 'btn-select': [' cn.', 2], 'btn-start': [' st.', 3]}, bios: ['./src/lib/neogeo.zip'], vars: {'fbneo-allow-depth-32': 'Disabled', 'fbneo-sample-interpolation': '4-point', 'fbneo-fm-interpolation': 'linear', 'fbneo-lowpass-filter': 'Disabled', 'fbneo-samplerate': '44100', 'fbneo-cpu-speed-adjust': '100', 'fbneo-diagnostic-input': 'Disabled'}},
    {ext: '.nds', script: CORE_BASE + 'nds2021.zip', btns: {'btn-1': ['A', 8], 'btn-2': ['X', 9], 'btn-3': ['B', 0], 'btn-4': ['Y', 1], 'btn-l': [' bl.', 10], 'btn-r': [' br.', 11], 'btn-select': [' sc.', 2], 'btn-start': [' st.', 3]}, bios: ['./src/lib/bios7.bin', './src/lib/bios9.bin', './src/lib/firmware.bin'], vars: {melonds_console_mode: 'DS', melonds_boot_directly: 'Enabled', melonds_screen_layout: 'Top/Bottom', melonds_screen_gap: '0', melonds_hybrid_small_screen: 'Disabled', melonds_swapscreen_mode: 'Disabled', melonds_randomize_mac_address: 'Disabled', melonds_touch_mode: 'Touch', melonds_dsi_sdcard: 'Disabled', melonds_mic_input: 'None', melonds_audio_bitrate: 'Low', melonds_audio_interpolation: 'None', melonds_use_fw_settings: 'Disabled', melonds_language: 'English'}},
    {ext: '.bin,.iso,.img,.cue,.pbp', script: CORE_BASE + 'ps1.zip', btns: {'btn-1': ['A', 8], 'btn-2': ['X', 9], 'btn-3': ['B', 0], 'btn-4': ['Y', 1], 'btn-l': [' bl.', 10], 'btn-r': [' br.', 11], 'btn-select': [' sc.', 2], 'btn-start': [' st.', 3]}, bios: ['./src/lib/scph5501.bin']},
];
var isRunning = false;
// ===== initCore ====
async function initCore(romFile) {
    ...
    if (scriptSource.endsWith('.zip')) {
        ...
        const jsFile = Object.keys(coreFiles).find(n => n.endsWith('.js')), wasmF = Object.keys(coreFiles).find(n => n.endsWith('.wasm'));
        scriptSource = URL.createObjectURL(new Blob([coreFiles[jsFile]], {type: 'application/javascript'}));
        window.wasmUrl = URL.createObjectURL(new Blob([coreFiles[wasmF]], {type: 'application/wasm'}));
    }
    return new Promise(async (resolve) => {
        ...
        window.Module = {
            ...
            async onRuntimeInitialized() {
                ...
                callbacks.forEach(([fP, cb, sig]) => fP(Module.addFunction(cb, sig)));
                Module._retro_init();
                ...
                if (!isArcade) {Module.HEAPU8.set(finalRomData, romPointer); loadInfo[1] = romPointer; loadInfo[2] = finalRomData.length;}
                ...
                function mainLoop() {
                    ...
                    if (window.isNetplaying) return; // Managed by netplay.js
